...
def test_codeserver_version(language, version_output):
    """Ensure codeserver is available in the PATH and that it returns the correct version"""
    LOGGER.info(f"Test that language {language} is correctly installed ...")
    client = docker.from_env()
    output = client.containers.run("illumidesk/codeserver:latest", f"{language} --version")
...
def test_invalid_cmd():
    ...
    with pytest.raises(ContainerError):
        ...
        client = docker.from_env()
        client.containers.run("illumidesk/codeserver", "foo --version")
