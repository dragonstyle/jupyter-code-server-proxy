...
def test_code-server_version(language, version_output):
    """Ensure code-server is available in the PATH and that it returns the correct version"""
    LOGGER.info(f"Test that language {language} is correctly installed ...")
    client = docker.from_env()
    output = client.containers.run("illumidesk/code-server:latest", f"{language} --version")
...
def test_invalid_cmd():
    ...
    with pytest.raises(ContainerError):
        ...
        client = docker.from_env()
        client.containers.run("illumidesk/code-server", "foo --version")
